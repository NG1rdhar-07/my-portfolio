  ...
  return (
    <div className="flex h-full flex-col bg-neutral-900">
      <div className="flex items-center gap-2 border-b border-white/10 bg-black/30 px-3 py-1.5 text-xs">
        <button onClick={() => setZoom((z) => Math.max(0.6, z - 0.1))} className="rounded p-1 hover:bg-white/10">
          <ZoomOut size={14} />
        </button>
        <span className="tabular-nums">{Math.round(zoom * 100)}%</span>
        <button onClick={() => setZoom((z) => Math.min(1.6, z + 0.1))} className="rounded p-1 hover:bg-white/10">
          <ZoomIn size={14} />
        </button>
        <div className="mx-2 h-4 w-px bg-white/15" />
        <button onClick={() => setPage((p) => Math.max(0, p - 1))} className="rounded p-1 hover:bg-white/10">
          <ChevronLeft size={14} />
        </button>
        <span>Page {page + 1} of {pages.length}</span>
        <button onClick={() => setPage((p) => Math.min(pages.length - 1, p + 1))} className="rounded p-1 hover:bg-white/10">
...
function ResumePage1() {
  return (
    <div className="min-h-[820px] bg-white p-10 text-neutral-900" style={{ width: 620 }}>
      ...
      <p className="mt-1 text-xs text-neutral-500">{profile.email} · {profile.location}</p>
      ...
      <h2 className="mt-4 text-sm font-semibold uppercase tracking-wide text-neutral-500">Experience</h2>
      {experience.map((e) => (
      ...
      <h2 className="mt-4 text-sm font-semibold uppercase tracking-wide text-neutral-500">Education</h2>
      {education.map((e) => (
        <div key={e.school} className="mt-1 flex justify-between text-sm">
          <span>{e.school} — <span className="text-neutral-600">{e.degree}</span></span>
          <span className="text-neutral-500">{e.period} · {e.detail}</span>
        </div>
      ))}
    </div>
  );
...
function ResumePage2() {
  return (
    <div className="min-h-[820px] bg-white p-10 text-neutral-900" style={{ width: 620 }}>
      ...
      <h2 className="mt-6 text-sm font-semibold uppercase tracking-wide text-neutral-500">Certifications & Research</h2>
